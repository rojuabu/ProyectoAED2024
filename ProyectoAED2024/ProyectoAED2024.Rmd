...
author:
  ...
  - name: Marc Velasco Mateu
    email: marcvem2@alumni.uv.es
    affiliation: UV
  - name: Rodrigo Juanes Busolo
    email: rojuabu@alumni.uv.es
    affiliation: UV
address:
  - code: UV
    ...
    country: España
abstract: |
  Este estudio analiza la evolución de los precios de la vivienda en España en relación con los salarios medios, con un enfoque particular en el precio por metro cuadrado en la ciudad de Valencia. El objetivo principal es identificar tendencias, destacar posibles disparidades económicas y proporcionar un análisis detallado basado en datos. Para ello, se han empleado técnicas de análisis exploratorio de datos (EDA) sobre tres conjuntos de datos: uno relacionado con salarios, otro con precios de vivienda en general, y un tercero específico sobre precios por metro cuadrado en Valencia. Los resultados de este trabajo buscan ser una base informativa para futuros estudios sobre la dinámica entre los salarios y el mercado inmobiliario en España.
...
```{r tamaño de los graficos, include=FALSE}
knitr::opts_chunk$set(fig.width=8, fig.height=4, dpi=150)
...
Primeramente, estudiemos el contenido del dateset Salario, formado por 4860 registros de 5 variables.

### Variables en el dataset

Inicialmente ejecutamos el comando str para observar el aspecto que tienen los datos. Además, este comando nos permite visualizar el tipo de dato que contiene cada columna, lo que nos permite comprobar si alguna de ellas debería ser convertida a otro tipo de dato más acorde a la información que contiene.
...
Además, dado que la variable de medias y percentiles es de tipo categórico y solo puede tomar un número finito de valores, esta fue convertida a factor. Del mismo modo, realizamos esta conversión con la variable sexo.
...
```{r datos faltantes en salario, include=FALSE}
# Por último comprobamos que no tenemos ningún dato perdido
vis_miss(Salario)
```

Utilizando la función `vis_miss`, descubrimos que, afortunadamente, no existen datos faltantes.
 
### Fin de la preparación del dataset

Finalmente guardamos el nuevo fichero con los cambios, para poder trabajar sin necesidad de volver a ejecutar este apartado.
...
```{r datos faltantes en vivienda, include=FALSE}
# Por último comprobamos si tenemos algún dato perdido
vis_miss(Vivienda)
```

Al usar la función `viss_miss`, encontramos algunos datos faltantes en la variable `Total`. Utilizando de nuevo el comando `str`, vemos que nuestro dataset no tiene ningún valor del IPV de las viviendas nuevas y de segunda mano en Ceuta y Melilla.
...
```{r str fotocasa, include=FALSE}
#Resumen de la estructura del dataset
str(fotocasa_compra)
```

Usando el comando `str`, observamos que la mayoría de las variables son de tipo numérico, excepto aquellas de coordenadas geográficas que son de tipo `character`, así como el distrito, el barrio o el año (siendo estas tres últimas convertibles a `character` o `factor`) y las referentes a coordenadas geográficas.
...
Las variables restantes sin comentar son de tipo numérico. Además, las únicas numéricas continuas son las del precio en los años de 2010, 2022 y máximos históricos. No obstante, antes de analizar con las métricas de resumen, hay que realizar un análisis de valores faltantes y duplicados, así como la presencia de valores anómalos que puedan afectar al posterior análisis.

### Duplicados y valores faltantes

```{r filas duplicadas o faltantes, include=FALSE}
...
No existen filas duplicadas dentro del conjunto de datos. Sin embargo, hay 18 valores nulos en columnas de máximos históricos y precios de metro cuadrado. Se verifica que barrios son los que tienen valores nulos y a qué distritos pertenecen. Se sabe que los valores faltantes son referentes a los años y precios de metro cuadrado.

```{r exposicion filas nulas, include=FALSE}
...
```{r exposicion distritos nulos, include=FALSE}
...
```{r exposicion distritos nulos especificos, include=FALSE}
...
Al explorar el dataset, se observa que de 3 distritos completos no se tienen datos. Esto supone un gran problema, sobreotodo cómo afrontarlo. No se puede eliminar porque suponen una pérdida importante en la representación del conjunto de datos. El mejor caso sería imputar. Si se tiene un dato no nulo perteneciente a algún barrio dentro de esos distritos se puede imputar ese mismo valor para el resto, pero no es el caso. Se puede aproximar el valor mediante barrios adyacentes, pero tampoco es una representación totalmente fiel.
...
Para el caso de 2010, no se ha encontrado información suficiente para realizar una imputación correcta, por lo que se decide aproximar por el precio de 2010 de aquellos distritos con un precio por metro cuadrado similar al de los distritos con valores faltantes en 2022. Además, el distrito que más se asemeja es el de L'Olivereta con 1162 €/m2. Vamos a imputar los valores faltantes de 2010 con este valor.
...
```{r busqueda media, include=FALSE}
...
```{r media 2010 la olivereta, include=FALSE}
...
```{r imputacion de valores, include=FALSE}
...
```{r consistencia del dataset, include=FALSE}
...
Existen 9 casos donde el máximo histórico es menor que el precio de 2010 o 2022. Esto puede deberse a un error humano o de recogida de datos. No puede ser un error de registro, pues todos han sido registrados durante el año 2023 y los registros más recientes son del año 2022. 
...
```{r arreglo de la consistencia, include=FALSE}
...
tabla_pdf <- tabla_combinada %>%
  knitr::kable(
    ...
    longtable = TRUE # Habilitar división en varias páginas si es necesario
  ) %>%
  kableExtra::kable_styling(
    latex_options = c("repeat_header", "scale_down"), # Repetir encabezados y ajustar escala automáticamente
    position = "center"
  ) %>%
  kableExtra::row_spec(0, bold = TRUE, background = "#FF5733", color = "white") %>%
...
grafico_final <- wrap_plots(graficos, ncol = 3)
...
# Crear tabla con kable para representar cada percentil con la mediana e IQR
...
tabla_metricas <- metricas_percentiles %>%
  kable(
    caption = "Métricas de Salario por Percentil: Mediana y Rango Intercuartílico para cada percentil",
    format = "latex",
    booktabs = TRUE,
    longtable = TRUE # Permite dividir en varias páginas si es necesario
  ) %>%
  kableExtra::kable_styling(
    latex_options = c("repeat_header", "scale_down"), # Repetir encabezados y ajustar escala automáticamente
    position = "center"
  ) %>%
  kableExtra::row_spec(0, bold = TRUE, background = "#FF5733", color = "white") %>% # Encabezado resaltado
  kableExtra::column_spec(2, bold = TRUE, color = "black") # Resaltar columna específica
...
Los datos numéricos del dataset Vivienda corresponden al IPV. Este es un índice calculado por el INE que busca explicar el precio de la vivienda. Así, dado que este dato ya es en sí profundamente procesado, no creemos que realizar un análisis como el anterior sea ocurrente, pues no seríamos capaces de interpretar correctamente los datos. Aún así, destacamos que el dataset Vivienda tiene una estructra muy similar al Salario, y por tanto, estudiar cosas como la frecuencia se volvería repetitivo.
...
Como observación, en el año 2022 existe un dato anómalo que se aleja mucho del resto de los datos. Este valor anómalo es el precio de 4029€/m2. Este valor es un 60% mayor que el precio máximo de 2010.

```{r barrio anómalo, include=FALSE}
...
Antes de empezar con el análisis explicativo, hacemos un inciso sobre la representación gráfica de las comunidades autónomas. Dado que representar todas y cada una de las comunidades autónomas en un solo gráfico puede empeorar el mensaje que intenta transmitir este, escogeremos un número reducido de regiones que mostrar.

Por supuesto, mostraremos los datos de toda España, así como los de la Comunidad Valenciana. Pero además, mostraremos los datos de Madrid, Cataluña, Canarias y Baleares, para tener así, otras regiones sobre las que comparar.
...
primer_analisis_filtrado <- primer_analisis %>%
  filter(`Comunidades autónomas` %in% particionCCAA)
...
Como se puede observar, las comunidades con mayor salario medio son Cataluña y Madrid. Por otro lado, las comunidades con peor sueldo medio a lo largo de los años son la Comunidad Valenciana y Canarias. Además se puede ver una clara tendencia al alza en los últimos años, lo que demuestra que el salario medio está subiendo año a año en nuestro país.
...
Como vemos, las comunidades con la menor diferencia de salario medio entre hombres y mujeres son las Islas Canarias y las Islas Baleares. Este dato es sorprendente, pues las Islas Canarias son una de las comunidades autónomas con menor salario medio.
...
## Análisis del archivo Precio M2 Valencia
...
```{r evolucion precio medio, echo=FALSE}
...
```{r diferencia por distritos, echo=FALSE}
...
```{r diferencia por barrios, echo=FALSE}
...
En el caso de los 10 barrios con mayor descenso de precio, se observa como diferencia varios pertenecientes a distritos con aumento de precio. Este es el caso del barrio de Natzaret en el distrito de Poblats Marítims o Cami Real en el distrito de Jesus. Esto da a entender que aunque haya distritos con aumento de precio, no quiere indicar que la tendencia de sus barrios haya sido de la misma manera.

Con este último análisis se concluye que la evolución de los precios en Valencia no ha sido uniforme, y que la localización de los barrios y distritos ha sido un factor determinante en la evolución de los precios.

# Conclusión

El análisis realizado nos ha permitido comprender mejor las dinámicas entre el precio de la vivienda y los salarios en España. Se observaron patrones relevantes que reflejan desafíos en términos de accesibilidad y sostenibilidad económica para la población en distintos puntos del país. Vemos como los salarios aumenta muy suavemente con el tiempo, mientras que a partir del 2015, el IPV ha aumentado a un ritmo frenético.

Este estudio resalta como consecuencia la importancia de contar con políticas efectivas que aborden estos desafíos y fomenta la reflexión sobre cómo las tendencias actuales pueden afectar al futuro de la población en España en calidad de una vivienda y salario digno.
