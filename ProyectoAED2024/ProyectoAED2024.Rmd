...
#Ajustamos los nombres de las comunidades
Salario$`Comunidades autónomas`[Salario$`Comunidades autónomas` == "Asturias, Principado de"] <- "Principado de Asturias"
Salario$`Comunidades autónomas`[Salario$`Comunidades autónomas` == "Balears, Illes"] <- "Illes Balears"
Salario$`Comunidades autónomas`[Salario$`Comunidades autónomas` == "Madrid, Comunidad de"] <- "Comunidad de Madrid"
Salario$`Comunidades autónomas`[Salario$`Comunidades autónomas` == "Murcia, Región de"] <- "Región de Murcia"
Salario$`Comunidades autónomas`[Salario$`Comunidades autónomas` == "Navarra, Comunidad Foral de"] <- "Comunidad Foral de Navarra"
Salario$`Comunidades autónomas`[Salario$`Comunidades autónomas` == "Rioja, La"] <- "La Rioja"
```
...
Ejecutamos de nuevo el comando str para ver los tipos de datos existentes en nuestro dataset Vivienda, y si estos son coherentes con la información que almacenan.

```{r str vivienda, echo=FALSE}
str(Vivienda)
```

Vemos como la variable "Total" que claramente representa un valor de tipo numérico es de tipo str, por lo que tendremos que cambiarla a una variable de tipo numérico.

Además, tanto la variable "General, vivienda nueva y de segunda mano" como "Índices y tasas" tienen un reducido número de valores posibles, por lo que transformaremos estas en factores.

Por último, ajustaremos la columna de comunidades y ciudades autónomas, pues igual que con el anterior dataset, los nombres de algunas comunidades aparecen fuera de orden.
...
```{r imputamos los datos faltantes, echo=FALSE}
...
#Escogemos los datos necesarios
Vivienda3<-Vivienda[Vivienda$`Comunidades y Ciudades Autónomas`=='Comunitat Valenciana' & Vivienda$`Índices y tasas`=='Media anual' & Vivienda$`General, vivienda nueva y de segunda mano`!='General',]
