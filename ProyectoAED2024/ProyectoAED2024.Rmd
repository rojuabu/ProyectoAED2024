...
#Análisis de dato de compra en la ciudad de Valencia por la plataforma Fotocasa
...
A la vista de la estructura del dataset, podemos observar que la mayoría de las variables son de tipo numérico, excepto aquellas de coordenadas geográficas que son de tipo character, así como el distrito, el barrio (que deberían transformarse a factor o caracter), la fecha de creación (que debería ser transformada a otra de formato DateTime) y las referentes a coordendas geográficas.
...
fotocasa_compra$AÑO_MAX <- as.character(fotocasa_compra$AÑO_MAX)
```
...
No hay filas duplicadas, eso es una buena señal. Ahora vamos a comprobar si hay valores nulos en el dataset.
...
Hay 18 valores nulos en columnas de máximos históricos y precios de metro cuadrado. Vamos a ver que barrios son los que tienen valores nulos y a qué distritos pertenecen. Sabemos que los valores faltantes son referentes a los años y precios de metro cuadrado.
...
Parece que todos son barrios pertenecientes al distrito de poblados del norte, sur y oeste. ¿Son todos los barrios del distrito o solo una parte de ellos?
...
Tras esto vamos a comprobar si los valores de precio máximo son los correctos. (Sólo los podemos cotejar con aquellos de los años 2010 y 2022 que son las columnas a las que tenemos acceso).



```{r}
# Comprobar que valores máximos son correctos (no son valores menores a los precios de 2010 y 2022)
fotocasa_compra %>% 
...
```
```{r}
#Mostrar esas filas (solo valores M2_2022, M2_2010 y M2_MAX)
fotocasa_compra %>% 
  filter(M2_MAX < M2_2010 | M2_MAX < M2_2022) %>% 
  select(BARRIO, M2_2022, M2_2010, M2_MAX, AÑO_MAX)
```
 
Vemos que hay 9 casos donde el máximo histórico es menor que el precio de 2010 o 2022. Esto puede deberse a un error humano o de recogida de datos. No puede ser un error de registro, pues todos han sido registrados durante el año 2023 y los registros más recientes son del año 2022. 

¿Cómo podemos imputar estos datos? No sería correcto asignar el valor máximo de la columna de 2010 o de 2022 porque no sabemos a ciencia cierta si son los valores máximos, al no tener registros de otros años. Si trabajáramos con otras fuentes de información sería posible hacer una imputación más precisa, pero en este caso no es posible. **Terminar de corregir**

 
## Plots de exploración de datos
```{r}
...
fotocasa_compra %>% 
  ...
  theme_minimal()
```
Viendo estos resultados, podemos observar que los precios de compra y de metro cuadrado están en una escala muy diferente a los máximos históricos. La distribución de los precios de 2010 son bastante más homogéneos. 
```{r}
...
fotocasa_compra %>% 
  ...
  labs(title = "Distribución de Variables Numéricas con Outliers") + 
  theme_minimal()
```
...
```{r}
valencia <- esp_get_munic_siane(munic = "^València$", epsg = 3857)

valencia_mask <- esp_getTiles(valencia, "IDErioja.Claro",
  mask = TRUE, crop = TRUE, zoommin = 1
)


ggplot() +
  geom_spatraster_rgb(data = valencia_mask) +
  theme_void() +
  labs(
    title = "Mapa Base de Valencia",
    caption = "CC BY 4.0 www.iderioja.org"
  )

#Plot in map of valencia values of 
